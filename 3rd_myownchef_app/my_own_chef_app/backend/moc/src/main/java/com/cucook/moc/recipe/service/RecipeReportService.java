...
import com.cucook.moc.recipe.vo.RecipeReportVO; // 내부 로직에서 VO 사용 가능

import java.util.List; // RecipeReportListResponseDTO 내부에서 List 사용

/**
 * 레시피 신고(tb_recipe_report) 기능과 관련된 비즈니스 로직을 정의하는 서비스 인터페이스입니다.
...
public interface RecipeReportService {
     ...
     */
    RecipeReportResponseDTO addRecipeReport(Long reporterUserId, RecipeReportRequestDTO requestDTO);
     ...
     */
    RecipeReportListResponseDTO getReportedRecipesByReporterUserId(Long reporterUserId);
     ...
     */
    RecipeReportResponseDTO getRecipeReportDetail(Long reportId, Long requestingUserId);
     ...
     * @throws IllegalArgumentException 해당 신고를 찾을 수 없거나 권한이 없을 경우
     */
    RecipeReportResponseDTO updateRecipeReport(Long reportId, Long reporterUserId, RecipeReportRequestDTO requestDTO);
     ...
     * @throws IllegalArgumentException 해당 신고를 찾을 수 없거나 권한이 없을 경우
     */
    boolean deleteRecipeReport(Long reportId, Long reporterUserId);
